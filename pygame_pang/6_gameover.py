...
# 화면 타이틀 설정
pygame.display.set_caption("Hee Pang") # 게임 이름
...
while running:
    ...
    # 100, 200 -> 180 160 140 ... 
    # 500, 200 -> 180 160 140 ...
    weapons = [ [w[0], w[1] - weapon_speed] for w in weapons] # 무기 위치를 위로 올리는 것

    # 천장에 닿은 무기 없애기
    weapons = [ [w[0], w[1]] for w in weapons if w[1] > 0]
    # w[1] (y값) 이 0보다 작으면 화면을 튀어나가는거고 0보다 크면 화면이 닿지 않은 상태이므로 닿지 않은 경우까지만 그리는 것
    ...
    for ball_idx, ball_val in enumerate(balls): 
        ...
        break # 안 쪽 for문에서 break를 만나면 여기로 진입 가능. 2중 for문을 한 번에 탈출
